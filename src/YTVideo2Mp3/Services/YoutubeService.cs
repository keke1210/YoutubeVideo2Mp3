...
using YTVideoToMp3.Extractor;
using YTVideoToMp3.Extractor.Helpers;

namespace YTVideoToMp3.Services
{
    public interface IYoutubeService
    {
        Task<YouTubeVideo> GetVideoAsync(string videoUri);
    }
    ...
    public class YoutubeService : IYoutubeService
    {
        ...
        public async Task<YouTubeVideo> GetVideoAsync(string videoUri)
        {
            ...
            string source = await _client.GetStringAsync(videoUri);

            return ParseVideos(source).First();
        }
                ...
                else
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(adaptiveMap))
                    {
                        ...
                        try
                        {
                            ...
                            var manifest = _client.GetStringAsync(dashmpdMap)
